...
function SetupScreen() {
	...
	// If user wants to go to chat and has valid providers, show chat interface
	if (showChat && hasValidProviders) {
		return (
				...
				<HeaderPage>
					<div className="flex items-center gap-4">
						<button
							...
							className="text-main-view-fg/70 hover:text-main-view-fg text-sm px-3 py-1 rounded-md border border-main-view-fg/20 hover:border-main-view-fg/40 transition-colors"
						>
							← Back to Setup
						</button>
					</div>
				...
				<div className="h-full px-4 md:px-8 overflow-y-auto flex flex-col gap-2 justify-center">
					<div className="w-full md:w-4/6 mx-auto">
						...
						<div className="flex-1 shrink-0">
							<ChatInput showSpeedToken={false} initialMessage={true} />
						</div>
					</div>
				</div>
			</div>
		...
		<div className="flex h-full flex-col relative overflow-hidden">
			...
			<div className="relative z-10 h-full px-8 overflow-y-auto flex flex-col gap-8 justify-center">
				...
				>
					...
					<div className="mb-12 flex justify-center">
						<div className="relative group">
							<img
								...
								className="w-18 h-18 md:w-18 md:h-18 drop-shadow-lg transition-all duration-500 group-hover:scale-110 group-hover:rotate-3"
							/>
							{/* Subtle glow effect */}
							<div className="absolute inset-0 w-28 h-28 md:w-36 md:h-36 bg-gradient-to-r from-blue-400/20 to-purple-400/20 rounded-full blur-xl opacity-50 animate-pulse"></div>
						</div>
					</div>

					{/* Welcome Message */}
					<div className="mb-16">
						<h1 className="font-editorialnew text-main-view-fg text-3xl md:text-5xl mb-6 font-bold bg-gradient-to-r from-main-view-fg to-main-view-fg/80 bg-clip-text text-transparent">
							{t('setup:welcome')}
						</h1>
					...
					{/* Continue to Chat Button (if has valid providers) */}
					{hasValidProviders && (
						<div className="mb-12">
							<button
								onClick={() => setShowChat(true)}
								className="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-10 py-4 rounded-xl font-semibold transition-all duration-300 text-xl shadow-lg hover:shadow-xl cursor-pointer"
							>
								🚀 Continue to Chat
							</button>
						</div>
